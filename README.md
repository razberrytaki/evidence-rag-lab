...
인용 검증, 실패 처리, 추적 가능성을 한 흐름으로 검증한다. 운영 환경에서 1천만
문서를 처리한다는 주장은 하지 않는다. 환각 0%를 보장하지도 않는다. 대신 큰 RAG
시스템을 설계할 때 어떤 질문을 먼저 던져야 하는지, 그 질문을 어떻게 작게 검증할
수 있는지를 보여주는 것이 목표다.
...
- [10M 규모 확장 추론 노트](docs/scale-to-10m.md): 지금 구현과 대규모 설계 사이의 간극
...
- public repo hygiene, claim scanner, publication gate

아직 production benchmark는 아니다. 큰 데이터셋, 접근 제어, privacy review,
index rebuild strategy, 장애 대응, 실제 부하 테스트는 별도 단계로 남아 있다.
