...
하는가”였다. 여기서 1천만 문서는 구현 완료 범위가 아니라 검색 품질, index memory,
trace 보존, 장애 대응 같은 압력을 한꺼번에 드러내기 위한 scale scenario다. 그래서
구현은 작은 공개 문서 세트로 재현성을 유지하고, 확장성 논의는 명시한 가정과 계산으로
분리한다.
...
scale scenario는 실제 load test 없이 주장할 수 없다. 대신 document 수, chunk 수,
embedding dimension, trace retention 같은 assumption을 명시하고 저장 공간과 index
pressure를 계산한다.

이 숫자는 성능 claim이 아니라 설계 대화를 위한 출발점이다.

관련 문서: [확장성 추론 노트](scale-to-10m.md)

## 검증한 것

- 15개 deterministic eval fixture가 모두 통과한다.
- 이 중 sample-runtime observation은 2건이며, insufficient-evidence 계열 negative
  guard를 우선 검증한다.
...
대표 수치는 README의 Evidence snapshot에 두고, 자세한 table은 generated report로
분리한다.
