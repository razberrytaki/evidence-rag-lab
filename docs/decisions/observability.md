...
정리된 추적 기록은 공개 저장소에 더 안전하면서도 디버깅에 유용하다. 원문 로그는 전체 LLM 요청, 문맥, LLM 응답을 노출한다.
...
추적 기록 정리를 예약 작업으로 옮기고, 추적 기록 행 삭제 전에 집계 지표를 내보낸다.
전체 LLM 요청이나 응답을 재생하지 않고도 순위 변화를 디버깅할 수 있도록 결합 순위,
재순위화 순위, 재순위화 점수, 거절 사유의 집계 경로를 강화한다.
