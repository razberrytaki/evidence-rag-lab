...
RAG 관측에는 추적 기록이 필요하다. 하지만 추적 기록은 실수로 프롬프트, 문맥, LLM 출력, 개인정보의 두 번째 저장소가 될 수 있다.
...
정리된 추적 기록 요약은 재현/디버깅 세부 정보 일부를 잃지만 공개 레포지토리에 더 안전하고 운영 개인정보 경계에 더 가깝다.

추적 기록 id 기반 결정적 샘플링은 로컬 테스트를 재현 가능하게 만들고, 실패가 무작위성 뒤에 숨는 것을 피한다.

짧은 보관 기간은 오래된 관측 데이터를 제한한다. 대신 장기 추세 분석은 원문 추적 기록이 아니라 집계 평가 보고서에서 나와야 한다.
...
`pnpm db:trace-cleanup`을 운영 스케줄러에 연결하여 삭제 전 집계 지표를 내보낸다.
추적 기록이 사설 네트워크 경계 밖으로 나가기 전에 더 엄격한 개인정보 가림 단계를 추가한다.

현재 확장성 예산은 일일 질의 `50,000`, 정리된 추적 기록당 `4096`바이트, 보관 기간
`7`일 기준으로 데이터베이스 부가 비용을 제외한 추적 기록 보관량을 `1.43 GB`로 추정한다.
